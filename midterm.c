...
int main(){
    ...
    f.colors =  mmap(NULL, f.palettecolors * sizeof(col), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANON, -1, 0);
    ...
    int *chunksizelist = (int*)  mmap(NULL, sizeof(int) * chunknum, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANON, -1, 0);
    BYTE *chunkcolorlist = (BYTE*)  mmap(NULL, sizeof(BYTE) * chunknum, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANON, -1, 0);
    int pixelcount = 0;
    csize = 0;

    int *ind = (int *)  mmap(NULL, sizeof(int) * 4, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANON, -1, 0);
    int total_size = 0;
    for(int i=0;i<chunknum;i++){
        ...
        total_size += 3;
        if (total_size == f.rowbyte_quarter[0]){
            ind[0] = i;
        }
        if (total_size == f.rowbyte_quarter[1]){
            ind[1] = i;
        }
        if (total_size == f.rowbyte_quarter[2]){
            ind[2] = i;
        }
        if (total_size == f.rowbyte_quarter[3]){
            ind[3] = i;
        }
        pixelcount += c.count;
    ...
    //int x=0;
    int y=1;
    f.rowbyte_quarter[0] *= 3;
    f.rowbyte_quarter[1] = pixelcount/2;
    f.rowbyte_quarter[2] = (pixelcount/4) * 3;
    printf("%d\n", f.rowbyte_quarter[0]);
    //int x=0;
    if(fork()==0){
        int x = 0;
    ...
    //     }
    // }
    // for(int i=f.rowbyte_quarter[1]; i<f.rowbyte_quarter[3]; i++){
    //     for(int j=0; j<chunksizelist[i]; j++){
